...
static void merge_payload(int n,uint32_t x[n],uint32_t pl[n],int step)
{
  int i;
  if (n == 1)
...
void sort_32b(int n, uint32_t x[n])
{
  if (n <= 1) return;
  sort_32b(n/2,x);
  sort_32b(n/2,x + n/2);
...
void sort_with_payload(int n, uint32_t x[n], uint32_t pl[n])
{
  if (n <= 1) return;
