...
static int weight_4608(uint32_t *v)
{
	int i, w = 0;
	for (i = 0; i < 4608; i++)
		w += (v[i>>5] >> (i&31)) & 1;
...
int decrypt_4608_96(unsigned char *e, const unsigned char *sk, const unsigned char *s)
{
	int i;
	...
	for(i=0;i<GFBITS;i++) s_priv[6][i]=0;
	for(i=0;i<GFBITS;i++) s_priv[7][i]=0;
	bm( locator , s_priv );
	locator[3][0] = 1;
	for(i=1;i<GFBITS;i++) locator[3][i]=0;
	...
	uint32_t cmp_eq = 0;
	for(i=0;i<6;i++) for(int j=0;j<GFBITS;j++) cmp_eq |= s_priv_cmp[i][j]^s_priv[i][j];
	check_synd = is_zero( cmp_eq );

	// check if weight is equal ?
	benes(error, sk + IRR_BYTES, 0);
	check_weight = weight_4608(error) ^ SYS_T;

	// check [4608-8192] == 0
	check_weight |= get_nonzero_mask_u32( &error[(4608/32)] , 256-(4608/32) );
