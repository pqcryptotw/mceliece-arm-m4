...
// required constants:
// const uint32_t power_128_u32[ 256 ][ GFBITS ] = { #include "powers_u32.data" };  // 16 KB
static void add_x128_u32( uint32_t values[][ GFBITS ] )
{
	int i,b;
	for (i = 0; i < 256; i++)
	for (b = 0; b < GFBITS; b++) 
		values[i][b] ^= power_128_u32[i][b];
...
void fft_p128_v8192(uint32_t values [][GFBITS], uint32_t poly [][GFBITS])
{
