...
static int weight_6688(uint32_t *v)
{
	int i, w = 0;
	for (i = 0; i < 6688; i++)
		w += (v[i>>5] >> (i&31)) & 1;
...
int decrypt_6688_128(unsigned char *e, const unsigned char *sk, const unsigned char *s)
{
	int i;
	...
	check_weight = weight_6688(error) ^ SYS_T;

	// check [6688-8192] == 0
	check_weight |= get_nonzero_mask_u32( &error[(6688/32)] , 256-(6688/32) );
