...
#if 119 == SYS_T
	for(i=0;i<13;i++) {
		poly[0][i] = (C[0][i]>>9)|(C[1][i]<<23);
		poly[1][i] = (C[1][i]>>9)|(C[2][i]<<23);
		poly[2][i] = (C[2][i]>>9)|(C[3][i]<<23);
		poly[3][i] = (C[3][i]>>9);
	}
	poly[3][0] |= 1<<(32-9);
...
//dump_gfs(C[3]);
	...
	for(i=0;i<13;i++) {
		poly[0][i] = (C[0][i]>>8)|(C[1][i]<<24);
		poly[1][i] = (C[1][i]>>8)|(C[2][i]<<24);
		poly[2][i] = (C[2][i]>>8)|(C[3][i]<<24);
		poly[3][i] = (C[3][i]>>8);
	}
