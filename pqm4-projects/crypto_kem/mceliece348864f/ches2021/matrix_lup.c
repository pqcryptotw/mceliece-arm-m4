...
int get_invitable_columns_32x64( uint64_t mat[] , uint16_t * idx_cols )
{
	for (int i = 0; i < 32; i++)
	{
		...
		idx_cols[i] = __builtin_ctzll(t);  // record the index of the least nonzero bit.
                // gcc 9.2.1 uses conditional move for checking zero of low-32 bits.

		// if pivot = 0, move lower rows to the pivot row
		for (int j = i+1; j < 32; j++) { uint64_t mask = (mat[i] >> idx_cols[i]) & 1; mask -= 1;    mat[i] ^= mat[j] & mask; }
		// row[j] -= row[i]* (row[j][idx[i]] )
...
int lup_decompose_2( uint32_t * mat , uint32_t * extra_mat , uint16_t * p , uint16_t * idx_cols, int n )
{
	...
	for (row = 0; row < n-32; row++) {  // normal procesure until the last 32 rows
		i = row >> 5;
		j = row & 31;
		uint32_t target_bit = (1<<j);
		for (k = row + 1; k < n; k++) {
...
//printf("j=%d. target_bit=%x, mask_0=%x\n", j , target_bit , mask_0 );
	...
	// get invertible columns among the last 32 rows
	uint64_t mat2[32];
	...
	// collect invertible columns among the laster 32 rows
	for(row = 0; row < n; row ++ ) {
	...
	for (row = n-32; row < n-1; row++) {    // continue processing the last 32 rows
		i = row >> 5;
		j = row & 31;
		uint32_t target_bit = (1<<j);
		for (k = row + 1; k < n; k++) {
