...
if( 0 != rem ) {
	...
	uint64_t mat2[32];
	for(i=0;i<32;i++) {
		uint32_t v = (mat[(n-32+i+1)*mat_width-2]>>rem)|(mat[(n-32+i+1)*mat_width-1]<<(32-rem));
		mat2[i] = ((uint64_t)v)|(((uint64_t)extra_mat[n-32+i])<<32);
	}
	...
	// collect columns
	for(row = 0; row < n; row ++ ) {
		uint32_t v = (mat[(row+1)*mat_width-2]>>rem)|(mat[(row+1)*mat_width-1]<<(32-rem));
		uint64_t t = ((uint64_t)v)|(((uint64_t)extra_mat[row])<<32);
		uint32_t d = 0;
		for(int i=0;i<32;i++) {
			uint32_t t1 = (t>>idx_cols[i])&1;
			d |= t1<<i;
		}
		mat[(row+1)*mat_width-1] = d>>(32-rem);
		mat[(row+1)*mat_width-2] &= (1<<rem)-1;
		mat[(row+1)*mat_width-2] |= d<<rem;
	}
} else {
	// get invertible columns
	uint64_t mat2[32];
